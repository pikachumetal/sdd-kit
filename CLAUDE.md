...
- `skills/<nombre>/SKILL.md` — las 13 skills del kit. `skills/sdd-templates/templates/` es la **fuente única** de las plantillas.
...
2. **Dogfooding** (Art. VII): los cambios no triviales del kit se arrancan con `sdd-start-feature`; artefactos en `.docs/sdd/specs/` con el naming estándar. Fixes pequeños deterministas → carril patch. El flujo se sigue con las skills del **working tree** (`skills/`), no con las de la caché del plugin instalado: si difieren, mandan las del working tree. Por eso la sesión se arranca con `./Start-KitSession.ps1` (comprobado en sesión interactiva en el patch 0027: el `Base directory` de la skill apunta al worktree; sin script, a la caché). Si no sale del script, el hook `SessionStart` del repo (`.claude/hooks/Test-KitSessionSource.ps1`) lo avisa al arrancar. Si la sesión no salió del script, **o si esta misma sesión editó la skill que vas a seguir** (el harness la sirve con el texto del arranque: ticket de la task 0057 §2), antes de ejecutar un paso de una skill cargada por el harness contrasta su texto con `skills/<nombre>/SKILL.md` de la rama. Si esta sesión **renombró o creó** la skill, el harness no la conoce hasta la próxima sesión: no invoques el nombre viejo, lee `skills/<nombre nuevo>/SKILL.md` con `Read` y sigue su checklist (ticket de la feature 0064 §2) — cinco reportes de skills cargadas desde la caché (tickets 0003, 0004, 0013, 0005 y patch 0024).
...
8. **Prompts para lanzar trabajo**: cuando una consulta da el prompt de una feature o un patch, lo encabeza con el nombre de rama y de worktree, con slug en inglés kebab-case: `feature/<id>-<slug>` si el id ya está reservado (una feature con su fila), `feature/<slug>` si no (un patch, que reserva al arrancar). Así se sabe qué es cada worktree en Orca (dev-lead, 2026-09-25).
