...
8. **Prompts para lanzar trabajo**: cuando una consulta da el prompt de una task o un patch, lo encabeza con el nombre de rama y de worktree, con slug en inglés kebab-case: `feature/<id>-<slug>` si el id ya está reservado (una task con su fila), `feature/<slug>` si no (un patch, que reserva al arrancar). Así se sabe qué es cada worktree en Orca (dev-lead, 2026-09-25).
