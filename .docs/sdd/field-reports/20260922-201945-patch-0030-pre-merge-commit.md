...
- **Coste**: cuatro turnos en total y un cierre en tres tramos (patch, ticket, merge del ticket).
- **Diagnóstico que el agente no pudo cerrar**: el dev-lead preguntó por qué nunca había pasado antes y propuso que otra sesión estuviera mergeando. El dato es real —entre los dos intentos, `develop` pasó de `91f41ec` a `2e4887c` y el worktree de otra task desapareció, así que otra sesión mergeó en ese rato—, pero no explica la denegación: el mensaje era del clasificador, no un `index.lock`. **Un agente no puede distinguir desde dentro «el entorno me deniega» de «hay concurrencia» si solo mira el texto del error**, y la pregunta del dev-lead lo demuestra.
- **Propuesta**: el paso declara la forma del informe cuando el merge se deniega: el comando literal en un bloque, el estado de la rama destino (hash actual, sin tocar), y que lo demás del cierre ya está hecho. Añadir a la tabla de gates de `control-profiles.md` que un merge autorizado por política y denegado por el entorno **no** se reintenta con otra herramienta ni se deja en silencio. Y que el informe distinga las dos causas con evidencia: cita el texto de la denegación y el hash de la rama destino, que delata si avanzó por otra sesión.
- **Criterio de aceptación**: GIVEN un cierre con `merge.into` declarado y un entorno que deniega `git merge`, WHEN el paso 6 termina, THEN el informe final cita el comando denegado, el texto de la denegación, el hash de la rama destino sin tocar y el resto del cierre como completo.
...
- **El hook cubrió justo el caso que la 0009 describe**: entre el cierre y el merge del ticket, otra sesión movió `develop` (`91f41ec` → `2e4887c`). El merge corrió la suite sobre esa base nueva y dio `Tests Passed: 310` en vez de los 302 que el agente había verificado en su rama: los 8 de más los trajo la otra task. Sin el hook, esa combinación no la habría ejecutado nadie.
