...
| 38 | **Ids de task autoincrementales para proyectos sin gestor de tickets.** Hoy `<id>` es el ticket del gestor y `0000` si no hay (`nombrado.md:7`, `spec-template.md:3`, `sdd-start-patch:36`); en los proyectos de trabajo con Azure DevOps va bien, en los personales todo sale `0000`. Verificado el coste: la rama es `feature/<ticket>` (`sdd-start-task:34`), así que **dos tasks sin ticket en worktrees paralelos chocan en `feature/0000`**, y la columna Task del estimation-log y el `<id>` del changelog quedan inservibles. `sdd-project-template` ya numera de hecho (0004, 0005, 0006a, 0008) por acuerdo con el dev-lead, que es lo único que el kit prevé: «sin gestor a la vista, la numeración se acuerda con el usuario» (`roadmap-fuente.md:5`) | dev-lead 2026-09-20 | `sdd-init-*`, `nombrado.md`, `sdd-start-task`, `sdd-start-patch`, `sdd-start-release`, `sdd-consult` | release-siguiente, y **antes de abrir worktrees**: afecta a esta misma release, cuyas tasks son todas `0000`. Diseño propuesto: modo de ids **por proyecto**, decidido en el bloque (d) de la entrevista (que ya pregunta por el gestor) y guardado en `sdd-kit.json` — `gestor` o `secuencia`. En modo secuencia el id lo **reserva el hilo principal al planificar** (fila del roadmap), no el agente de la task: es la única forma de que dos worktrees no cojan el mismo número; para una task no planificada, script determinista en `sdd-templates/scripts/` (mayor id en `specs/` y roadmap + 1, cuatro dígitos). Hay que reconciliarlo con «los ids de ticket no se inventan» (`sdd-consult:37`, `sdd-start-release:58`): en modo secuencia el generador es el kit, no es inventar. **Decidido por el dev-lead (2026-09-20)**: la numeración se decide en las `sdd-init-*` y el resto de skills usan lo decidido; **sin sufijos** tipo `0006a` —salieron de una recomendación del agente al partir una task y el resultado es feo—: una task partida recibe el **siguiente id libre de la secuencia** y la relación vive en el frontmatter de su spec (`parent: <id>`) y en el roadmap, no en el id. Abierto: ¿una sola secuencia para tasks y patches? | **release-siguiente** |
| 39 | **Skill `sdd-feedback`: que el ticket de mejora del kit se genere solo al acabar una tarea**, en `.docs/sdd/feedback/` del proyecto, con plantilla propia y **escrito para que lo lea un agente**, no una persona. El dev-lead lleva cinco tickets pedidos a mano, explicando cada vez el formato | dev-lead 2026-09-20 | skill nueva, `sdd-templates`, cierre de `sdd-end-task` / `sdd-end-patch` / `sdd-init-*` | release-siguiente y **de las primeras**: alimenta todo lo demás — la retro de esta acta mide que el uso real devolvió más en cuatro días que las campañas en semanas. Entradas de diseño: (a) **los cinco tickets ya recibidos son el baseline RED** y salieron buenos sin skill, así que por el Art. I la skill debe ser **fina y el peso va en la plantilla** («un artefacto bien formado es guidance», T4/T5): lo que aporta no es calidad de contenido sino no tener que pedirlo, sitio y forma fijos para cosecharlos con un glob, y las reglas que ningún ticket traía de serie; (b) la plantilla fija lo que hizo útiles a los mejores: versión del kit y de superpowers, fichero y paso del kit por hallazgo (**verificable**), qué pasó · por qué el kit no lo evitó · propuesta · escenario RED, coste en reloj y tokens, **qué hizo el agente por iniciativa propia que el kit no pedía**, «funcionó, no tocar», y errores del ejecutor separados de los huecos del kit; (c) **salida honesta obligatoria**: «sin hallazgos» es un resultado válido — una skill que siempre pide fricciones las inventa (T18: «una línea que pide un número invita a inventarlo»); (d) **privacidad**: en proyectos de cliente el ticket describe el comportamiento del kit, no el dominio ni el código del cliente, porque acaba copiado en el repo del kit; (e) se genera **en la misma sesión que ejecutó la tarea** —al limpiar el contexto ese conocimiento se pierde—, así que además de invocable a mano conviene que el cierre de cada carril la ofrezca; (f) **requisito del dev-lead para la carpeta**: fuera de `specs/`, una sola y conocida, para poder ponerla en `.gitignore` y saber dónde están todos; el nombre queda abierto — `feedback/` choca con `releases/vX.Y.Z/feedback.md`, que es el acta; candidato `kit-feedback/`; (g) el destino final en el repo del kit ya existe: `field-reports/` | **release-siguiente** |
...
- **Qué corregir**:
  ...
  - **39 peticiones para una sola release contradice «release pequeña primero»** (T15): la siguiente debe partirse en tramos con smoke por tramo.
