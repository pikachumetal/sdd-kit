...
- **Destinatario**: persona o grupo, distinto de quien hace la release, que la recibe. Lo declara `release.hasRecipient` en `sdd-kit.json`. Sin destinatario no hay release notes ni email, y el merge y el tag se ejecutan sin segunda ronda si el usuario ordenó el cierre y escribió la versión ([`release-flow`](capabilities/release-flow.md)).
