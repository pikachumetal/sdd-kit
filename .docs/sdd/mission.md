...
- **`unattended`** (desatendido): sin paradas hasta terminar la release. Solo con información suficiente (funcional y planificación de `sdd-plan`); si una task no la tiene, se aparca como bloqueada. La spec la aprueba el agente con las decisiones registradas; un desvío se resuelve con la opción más conservadora y se registra, y si bloquea, la task se aparca. La validación se aplaza a un único smoke de release. Al final, un solo informe; el merge a main y el tag siguen siendo del dev.
