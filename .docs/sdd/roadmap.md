...
| T2 | Progressive disclosure de las skills largas: `SKILL.md` (gates, enrutado, tablas) + `references/` leídos en el punto de uso; borrar solo prosa muerta; re-test Art. I. Absorbe el pendiente "crea un todo por paso" en 8 skills (TodoWrite retirado en Claude Code v2.1.233). **Método de la campaña de compresión de superpowers 6.2.0** (`RELEASE-NOTES.md`, leído el 2026-09-07): se borra prosa de recapitulación, "social proof" y venta de beneficios dirigida a quien ya invocó la skill; **todo argumento que sostiene conducta se conserva**, doblado en fila de tabla de racionalizaciones o movido a su punto de uso. Cada corte se micro-testa con subagentes contra un control sin guidance, y el único corte que degradó conducta medida se rehízo en vez de publicarse: borrar la sección "Why Order Matters" de TDD bajó el test-first de 8/10 a 5/10 bajo presión ("just write it, tests after"), corroborado en Claude y Codex, así que sus rebuttals sobrevivieron como filas de racionalización. Lectura para el kit: la tabla de racionalizaciones **es** la forma comprimida de un argumento, no decoración | ítem "recorte" (antes <500 palabras) · deuda de tamaño · ítem 3 | ⏳ |
| T3 | Workflow y ejecución: `subagent-driven-development` por defecto con política de modelos forzada desde el plan (campo `Modelo` por task, aprobado con el plan; el más barato que resuelve bien; nunca `fable` ni `opus xhigh` por defecto), `test-driven-development` y `requesting-code-review` explícitos, cierre = merge a `develop` local y worktree borrado por el usuario. Hoy la política vive solo en Alybo (`.claude/CLAUDE.md:50-75`) y el kit dice lo contrario. Debe **re-testar el bloque "Restricciones globales"** de `plan-template` con un plan multi-task y ejecutores que solo ven su task (residual de T1) | consulta 2026-09-07 · práctica real en Alybo | ⏳ |
...
| --- | --- | --- |
| `sdd-start-task` con **1503 palabras** (965 antes de modo-lite, 1403 antes de T1). Relectura 2026-09-07: superpowers no acorta skills (`brainstorming` 2324, `subagent-driven-development` 4823), mantiene pequeño lo siempre cargado (`using-superpowers` 485) y parte el resto en `SKILL.md` + auxiliares leídos en el punto de uso; el límite de tamaño aplica a lo que se carga en toda sesión, no al `SKILL.md` | Medio | T2 |
