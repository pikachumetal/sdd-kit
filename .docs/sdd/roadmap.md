...
| T12 | Review reforzada opt-in por task: campo `Review reforzada` en el plan (motivo: auth, roles y permisos, concurrencia, dinero o datos personales, sin tests RED previos), tope de tres lentes, triage sin refutador automático, MEDIUM al fix, registro en el walkthrough. **Condicionada al RED con bug plantado** (transición de estado sin guarda): si la review de superpowers lo caza, no se añade nada. Fuente: [research.md de T11](specs/20260909-131802-task-0000-gates-y-reviews/research.md) (retos SifAcademy y SifRest) | propuesta del dev-lead 2026-09-09 | ⏳ lite, tras T11 y antes de cerrar la release |
| T13 | Disparo de las skills: medir en headless si `sdd-start-task` se invoca sola ante «implementa la task N» (la `description` es el disparador; si no dispara, afinar con las frases reales del equipo y que `init-*` ponga la regla en el `CLAUDE.md` del proyecto); Gate 1 con dos vías explícitas (invocación sola → contexto y parar; con enunciado → contexto y seguir) | consulta 2026-09-09 (gate de validación de T11) | ⏳ lite, tras T12 y antes de cerrar la release |
