...
| T5 | `sdd-init-brownfield` con ideas de OpenSpec: `funcional.md` por capacidad generado del código, y **`funcional.md` vivo con delta** (`ADDED` / `MODIFIED` / `REMOVED` en la spec, fusionado por `sdd-end-task`). Hoy el kit lo declara en cuatro sitios y ninguna skill lo escribe; el propio kit tampoco lo tiene (dogfooding) | backlog OpenSpec | ⏳ cambio de convención → Art. IV: spec dedicada |
| T6 | `spec-template`: quick test de qué NO va en una spec y escenarios GIVEN/WHEN/THEN con RFC 2119, para que el smoke de `sdd-end-task` verifique contra algo | backlog OpenSpec | ⏳ |
