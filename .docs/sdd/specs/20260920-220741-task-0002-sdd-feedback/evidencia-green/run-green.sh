...
for N in 1 2; do
  bash "$BASE/run-subject.sh" molde-cliente    "$KIT" "E1-$SUF-$N" 50 "$P_E1" feature/0007 &
  bash "$BASE/run-subject.sh" molde-cliente    "$KIT" "E2-$SUF-$N" 30 "$P_E2" feature/0007 &
  bash "$BASE/run-subject.sh" molde-limpia-v2  "$KIT" "E3-$SUF-$N" 25 "$P_E3" feature/0004 &
  wait
done
