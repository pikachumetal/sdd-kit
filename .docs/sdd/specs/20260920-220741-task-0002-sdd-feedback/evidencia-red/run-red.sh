...
for N in 1 2; do
  bash "$BASE/run-subject.sh" molde-cliente "$KIT" "E1-$SUF-$N" 45 "$P_E1" &
  bash "$BASE/run-subject.sh" molde-cliente "$KIT" "E2-$SUF-$N" 30 "$P_E2" &
  bash "$BASE/run-subject.sh" molde-limpia  "$KIT" "E3-$SUF-$N" 25 "$P_E3" &
  wait
done
