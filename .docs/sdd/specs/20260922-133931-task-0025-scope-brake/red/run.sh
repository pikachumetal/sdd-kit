#!/usr/bin/env bash
# RED previo a la spec de la 0025: un escenario por frente, dos sujetos cada uno.
set -u
...
for i in ${SUBJECTS:-1 2}; do
  bash "$S" "$KIT" "e1-red-$i" "t2red t2" in_progress "$E1" "$O" &
  bash "$S" "$KIT" "e2-red-$i" "t2red" in_progress "$E2" "$O" &
  bash "$S" "$KIT" "e3-red-$i" "t2red t2" done "$E3" "$O" &
  E4_DEVELOP=1 bash "$S" "$KIT" "e4-red-$i" "" pending "$E4" "$O" &
done
wait
