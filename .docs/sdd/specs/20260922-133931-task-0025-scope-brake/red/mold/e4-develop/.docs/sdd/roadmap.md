...
| 0009 | Validar el formato de la franja horaria (`HH:MM-HH:MM`) en `libres`, `reservar` y `cancelar` (la hora de `cancelar` se valida como `HH:MM`): si viene mal, mensaje de error en castellano y no se consulta ni se reserva nada | `src/app.js`, `test/app.test.js` | 🔄 en curso |
