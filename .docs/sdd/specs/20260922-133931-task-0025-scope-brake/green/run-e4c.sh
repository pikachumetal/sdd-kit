#!/usr/bin/env bash
# GREEN de la 0025, tercera vuelta: comprobación de fila fundida con los tests RED y bucle de fix excluido del freno.
set -u
...
for i in ${SUBJECTS:-1 2}; do
  bash "$S" "$KIT" "e5c-green-$i" "t2red" in_progress "$E5" "$O" &
  E4_DEVELOP=1 bash "$S" "$KIT" "e4c-green-$i" "" pending "$E4" "$O" &
done
wait
