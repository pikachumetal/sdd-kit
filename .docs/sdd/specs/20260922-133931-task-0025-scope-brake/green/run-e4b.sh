#!/usr/bin/env bash
# GREEN de la 0025, segunda vuelta: E4 tras sacar la comparación de fila a línea propia, y E5 como control.
set -u
...
for i in ${SUBJECTS:-1 2}; do
  bash "$S" "$KIT" "e5b-green-$i" "t2red" in_progress "$E5" "$O" &
  E4_DEVELOP=1 bash "$S" "$KIT" "e4b-green-$i" "" pending "$E4" "$O" &
done
wait
