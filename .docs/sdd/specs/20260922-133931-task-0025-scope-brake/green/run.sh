#!/usr/bin/env bash
# GREEN de la 0025: los cuatro escenarios del RED, intactos, más el control E5.
set -u
...
for i in ${SUBJECTS:-1 2}; do
  bash "$S" "$KIT" "e1-green-$i" "t2red t2" in_progress "$E1" "$O" &
  bash "$S" "$KIT" "e2-green-$i" "t2red" in_progress "$E2" "$O" &
  bash "$S" "$KIT" "e3-green-$i" "t2red t2" done "$E3" "$O" &
  bash "$S" "$KIT" "e5-green-$i" "t2red" in_progress "$E5" "$O" &
  E4_DEVELOP=1 bash "$S" "$KIT" "e4-green-$i" "" pending "$E4" "$O" &
done
wait
