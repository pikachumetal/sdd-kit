...
const describe = (block) => {
  ...
  if (block.name === "Agent") {
    const headings = String(input.prompt ?? '').split('\n').filter((l) => l.startsWith('## ')).join(' · ');
    return `subagent_type=${input.subagent_type ?? "(sin tipo)"} model=${input.model ?? "(sin modelo)"} · ${input.description ?? ""} · secciones: ${headings || '(ninguna)'}`;
  }
