...
- [ ] **Step 2: `sdd-end-task`** *(si E1 falla)* — insertar como paso 1-bis, tras `walkthrough.md`: «**Fusionar el delta en `funcional/`** — cada `ADDED` se añade a `funcional/<capacidad>.md`, cada `MODIFIED (antes: …)` sustituye el requisito con ese título, cada `REMOVED` lo quita. **Nunca crees una capacidad que la spec no declare** en "Decisiones a validar". Sin delta → no aplica y se dice.»
- [ ] **Step 3: `sdd-consult`** *(si E2 falla)* — en el paso 1: «una pregunta de comportamiento ("¿qué hace hoy…?") lee `funcional/<capacidad>.md` antes que las specs o el código: es la verdad viva».
- [ ] **Step 4: `sdd-init-brownfield`** *(si E3 falla creando o volcando)* — ya cubierto por el rename del Step 1 si la frase «NO se crea» basta; si el RED muestra volcado pese a ella, se añade la fila de racionalización con su frase textual.
