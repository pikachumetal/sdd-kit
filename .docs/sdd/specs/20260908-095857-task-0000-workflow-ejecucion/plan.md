...
- [x] ~~**Step 3: `plan-template.md` — TDD por predicado.**~~ **NO SE EJECUTA.** El RED (F-positivo, 2/2) muestra que el baseline ya hace test-primero sin que nadie nombre la skill: guidance sin fallo que la respalde está prohibida por el Art. I. Decisión registrada en `tests/workflow-ejecucion-red.md`.
- [ ] **Step 3-bis: `sdd-start-task` — traspaso de las Restricciones globales.** Añadir al paso 6, tras la mención al default: «Al despachar cada task a un subagente, **incluye en su encargo el bloque "Restricciones globales" del plan íntegro**: el ejecutor solo ve su task y no hereda lo que no se le entrega (medido en `tests/workflow-ejecucion-red.md`, F4).» Sustituye a la opción de repetir los campos en cada task, descartada por duplicación.
