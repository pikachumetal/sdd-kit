#!/usr/bin/env bash
# Sujeto headless a un turno en el paso de rama del cierre, sobre el molde de la task 0009 (repo bare con worktrees)
# y un remoto con URL de GitHub que no acepta el push (sin credenciales ni prompt).
# Uso: subject.sh <kit> <etiqueta> <escenario> <petición> <salida>
#   escenario: a (task, merge.push true) · b (patch, merge.push true) · c (task, sin merge.push)
set -u
BASE="$(cd "$(dirname "$0")" && pwd)"
M0="$(cd "$BASE/../../20260923-120510-task-0009-merge-close/red/m" && pwd)"; M1="$BASE/m"
...
layer base "$M0"
[ "$SC" != c ] && sed -i 's/"removeWorktree": false}/"removeWorktree": false, "push": true}/' "$SRC/.docs/sdd/sdd-kit.json"
