...
- 2026-09-23 — El merge del cierre chocó en `changelog.md`, `roadmap.md` y `estimation-log.md` con la task 0016, fusionada en paralelo. Con el sí del dev-lead («ya puedes hacer»), `develop` se integró en la rama con un merge de sincronización (`e920e0a`): las dos entradas del changelog, la fila 0044 de la rama más la 0046 de `develop` y el log regenerado. Ese merge queda fuera de la forma 2 + N, como prevé la spec. — agente
- 2026-09-23 — Al relanzar `Invoke-SddMerge.ps1`, el `pre-merge-commit` falló en el worktree temporal por un ancla de `tests/CommitMilestones.Tests.ps1`: `(?m)^## <título>$` no casa con finales CRLF, que es como llega la referencia en un checkout nuevo en Windows, mientras que en el worktree de la task estaba en LF. Se corrigió con `\r?$`. El script lo reportó como «merge: conflicto en .», porque trata cualquier fallo del merge como conflicto (deuda en el roadmap). Es un commit del hilo de una línea de test, revisado en el hilo (menos de ~20 líneas), que va detrás del merge: no se puede juntar con el cierre a través de un merge (guarda). — agente
