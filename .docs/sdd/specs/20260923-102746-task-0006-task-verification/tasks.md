...
- **Task 2**: `tasks-template.md` no estaba en la lista de ficheros del plan; su línea de «Verificación por task» decía «según la política del proyecto» y ahora remite al campo «Verificación» de la task. Sin cambio de spec: es la misma regla en la plantilla hermana. **Fix de la revisión final**: se editó sin su propia ancla RED→GREEN (Art. I, sin excepción por tamaño) — el bloque `Describe 'Plantilla de tasks'` de `tests/TaskVerification.Tests.ps1` faltaba. Rojo retroactivo (el texto viejo, «según la política del proyecto», habría fallado el `Should -Not -Match` de ese mismo `It`): documentado aquí en vez de revertir y volver a editar solo para capturar la salida. Verde: `Invoke-Pester tests/TaskVerification.Tests.ps1` → 9/9.
