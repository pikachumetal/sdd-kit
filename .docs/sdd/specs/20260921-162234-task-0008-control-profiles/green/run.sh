...
S="$BASE/../red/subject.sh"; O="$BASE/o"; G=../green
...
bash "$S" "$KIT" "$G/m-cm" "e6-green-$I" "Cierra la task 0009." "Validado: probé \`libres 24:00-24:30\` y da el error." "$O" &
MAX_TURNS=60 bash "$S" "$KIT" "$G/m-un" "e8-green-$I" "Trabaja las tasks de la release 0.4.0 hasta terminarlas." "" "$O" &
...
FEATURE_BRANCH=develop bash "$S" "$KIT" "$G/m-rel" "e10-green-$I" "Cierra la release." "Sí, v0.4.0. Validé el smoke: probé la franja de \`libres\` de la 0009." "$O" &
