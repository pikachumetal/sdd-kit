...
## Ruling — desvío del plan

`src/slots.js` estaba marcado «NO se tocan» (compartido con la reescritura del módulo de informes en task 0008, otro worktree). El smoke del Step 3 reveló que su regex aceptaba hora `24` (`2[0-4]`), dejando pasar `24:00-24:30` como válida — contradice el THEN de la spec, que lo lista como ejemplo a rechazar. No es un cambio de spec, es un defecto que impedía cumplirla: se corrigió el regex a `2[0-3]` en commit propio, con test añadido, en vez de duplicar la validación de rango en `app.js`. Pendiente: avisar a quien lleve task 0008 del cambio en ese fichero compartido.
