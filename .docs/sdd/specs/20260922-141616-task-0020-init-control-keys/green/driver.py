# Copia del lanzador de la task 0012: remoto de host real sin push y molde de migración con main y develop.
...
def prepare(mold, label):
    ...
    if mold == "e1-template":
        ...
        git(run, "remote", "add", "origin", "https://github.com/example-org/statusline.git")
    elif mold == "g3-migration":
        git(run, "init", "-q", "-b", "main"); git(run, "add", "-A")
        git(run, "commit", "-q", "-m", "docs(sdd): onboarding con el kit 1.1.0")
        git(run, "checkout", "-q", "-b", "develop")
        git(run, "remote", "add", "origin", "https://github.com/example-org/statusline.git")
    return run
...
def main():
    ...
    for turn in range(1, int(max_turns) + 1):
        ...
        if message.strip().rstrip(".") == "FIN" or not message:
