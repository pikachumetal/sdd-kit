...
task: 0062
title: sdd-roadmap, una sola puerta de entrada al roadmap, con el carril proposal
mode: full
status: approved
created: 2026-09-25
...
- Dominio: si la sección de release de `sdd-roadmap` (decisión 13) recoge lo que `sdd-start-release` hacía y aún sirve, y si el reparto sin estado (decisión 6) deja la propuesta legible con varias features cerradas (señal: capacidad nueva + MODIFIED)
...
- **Aceptado** — (dominio, Importante) las racionalizaciones probadas de `sdd-start-release` se perdían → decisión 13 las porta a `sdd-roadmap` y dice dónde se miden.
...
8. **La definición cambia a mitad**: `sdd-roadmap` añade una enmienda fechada (qué regla cambia, el valor anterior y el nuevo, con su ejemplo con datos, y quién lo pidió) y no reescribe «Reglas de negocio» ni el acta. Re-parte **solo lo pendiente**: una feature cerrada (✅ o 🧪) no se reabre, y lo que el cambio le pida va a una fila nueva. Una feature en marcha (rama abierta o 🔄) tampoco se toca: el trabajo va a una fila nueva «tras» ella, como ya hacía replanificar.
9. **`sdd-roadmap` no arranca nada**: ni rama, ni carpeta de task, ni spec, ni código. Termina en el roadmap, y en `proposal.md` si toca. Arrancar es `sdd-start-task` o `sdd-start-patch`, y el mensaje final dice con cuál y qué fila va primero.
10. **Reglas del roadmap que fija `sdd-roadmap`**:
...
14. **Retirada de `sdd-start-release`**:
    ...
    - Se actualizan los tests que la leen (`TaskIds`, `ReleaseFlow`, `MigrationInitParity`) para que lean `sdd-roadmap`.
    ...
    - `migrations/v1.2.0.md` gana un paso sin gate que solo avisa: «`sdd-start-release` ya no existe: "prepara la release" y replanificar son `sdd-roadmap`».
...
**ADDED — Una petición de planificar entra por `sdd-roadmap`**
...
- 2026-09-25 — Decisión 2: la skill pasa de `sdd-plan` a `sdd-roadmap`. `sdd-plan` choca con `plan.md` y con `superpowers:writing-plans` (lo señaló otro dev del equipo); se descartaron `sdd-epic` (obliga a pensar en niveles, y pediría renombrar el carril `proposal`) y `sdd-proposal` (choca con el carril `proposal` y con `/opsx:propose` de OpenSpec). El carril `proposal` no cambia. El RED y el GREEN midieron la skill con el nombre `sdd-plan`; el enrutado se vuelve a medir con el nombre nuevo — aprobada: «no me acaba de gustar del todo roadmap, pero es la que le encuentro menos pegas de las 3... así que tiramos con roadmap»
