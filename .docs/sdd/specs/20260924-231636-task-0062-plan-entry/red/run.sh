#!/usr/bin/env bash
# Campaña de la 0062: el RED y el GREEN comparten techo. Para si existe `stop` junto a este lanzador, si se llega a SUBJECT_CAP sujetos o si el coste acumulado pasa de COST_CAP.
...
SUBJECT_CAP="${SUBJECT_CAP:-50}"
COST_CAP="${COST_CAP:-25}"
...
for sc in ${SCENARIOS:?define SCENARIOS}; do
  ...
  KIT="$KIT_DIR"; [ "$sc" = p6 ] && [ -n "${KIT_DIR_P6:-}" ] && KIT="$KIT_DIR_P6"
  bash "$BASE/subject.sh" "$KIT" "$sc-${SUBJECT:-1}" "$sc" "$O" &
  pids+=($!)
  launched=$((launched + 1))
done
[ ${#pids[@]} -gt 0 ] && wait "${pids[@]}"
echo "sujetos de la campaña 0062: $(subjects) · coste acumulado: $(spent) \$"
