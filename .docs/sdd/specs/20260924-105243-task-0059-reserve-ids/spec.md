...
7. **Un contador por repositorio.** En un monorepo con varios proyectos en `sequence`, todos comparten el contador. Eso deja huecos en la numeración, pero nunca produce una colisión. Queda como límite declarado, junto al de una sola máquina.
...
- THEN devuelve por salida estándar el siguiente id en cuatro dígitos: el mayor valor entre el contador del repositorio y el mayor id encontrado en `.docs/sdd/specs/`, en `.docs/sdd/roadmap.md` y en los nombres de rama locales y remotos, más uno
...
- **Dónde viven los datos**: el modo, en `.docs/sdd/sdd-kit.json` (`ids.mode`). El último id consumido, en `<git-common-dir>/sdd-ids`, compartido por todos los worktrees del repositorio y fuera del árbol versionado. La fila del roadmap y la rama siguen siendo donde se ve cada reserva.
- **Límites**: cuatro dígitos (`0001`–`9999`); una reserva que pasaría de `9999` falla sin reservar. `-Count` va de 1 a 99. Una sola máquina: con varias máquinas en `sequence` haría falta un cerrojo en el remoto o el modo `tracker`. Hay un contador por repositorio, así que dos proyectos en `sequence` dentro de un mismo monorepo lo comparten, con huecos y sin colisiones.
