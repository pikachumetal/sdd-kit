...
1. El buscador reconoce un código de reserva con la expresión `^(?:[A-Za-z]+-)?R-\d{4}$` — cubre el propio (`R-0042`) y el importado con el prefijo de la sede de origen delante (`MAD-R-0042`), tal como lo define `capabilities/bookings.md` — y, si la consulta encaja, busca por código en vez de por cliente. Corrijo aquí el regex del borrador (`^R-\d{4}$`, que solo cubría el propio): no matcheaba el importado que la decisión 2 exige reconocer.
...
- AND la búsqueda no distingue mayúsculas (`r-0042` encuentra `R-0042`, `mad-r-0042` encuentra `MAD-R-0042`)
