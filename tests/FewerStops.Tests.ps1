BeforeAll {
  ...
  function Get-GateRow([string]$Point) {
    $text = Get-KitFile 'skills/sdd-start-feature/references/control-profiles.md'
...
Describe 'Task 1 — carril de task' {
  ...
  It 'el paso 7 pregunta sola la decisión del usuario antes de la validación' {
    Assert-Literal (Get-SkillStep 'sdd-start-feature' 7) @('**sola, en su propio turno**', 'presenta la validación en el turno siguiente')
  }

  It 'el paso 7 toma el «sí» sin detalle como validación' {
    Assert-Literal (Get-SkillStep 'sdd-start-feature' 7) @('Un «sí» sin detalle', '**es validación**', 'no detalló qué probó')
  }
  ...
  It 'sdd-end-feature acepta el «sí» sin detalle y fija su línea en el walkthrough' {
    Assert-Literal (Get-SkillStep 'sdd-end-feature' 0) @('«sí» sin detalle')
    Assert-Literal (Get-SkillStep 'sdd-end-feature' 1) @('`Validado: <fecha> · «<frase literal>» · no detalló qué probó`')
  }
}
