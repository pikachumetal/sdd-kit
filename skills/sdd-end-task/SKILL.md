...
2. **Tiempo real** *(si existe `.docs/sdd/estimation.md`)* — estimado vs real en el walkthrough; nunca en
   ...
   Los tokens y el coste no se escriben de memoria: ejecuta `pwsh -NoProfile -File <Base directory de sdd-templates>/scripts/Measure-SessionTokens.ps1 -Path <worktree> -Branch <rama de la task>` y pega sus tres líneas (`Tokens del hilo`, `Tokens de subagentes`, `Coste de la sesión`) sin retocar las cifras. Lo anterior a crear la rama no cuenta: dilo en la línea del hilo. Fuera de Claude Code no hay transcripts que leer y las tres dicen «no medido». Con el script en el kit pero sin este paso, 2 de 2 sujetos escribieron «no medido» (`tests/session-tokens-red.md`).
...
4. **Aprendizajes → docs vivos** — cada aprendizaje del walkthrough se vuelca donde vive. Destino por tipo
   ...
   si falla, corrige la fusión o el bloque «Capacidades» de la spec, nunca el validador.
...
8. **`roadmap.md`** — marcar el módulo/tarea. `✅` exige validación documentada en el walkthrough; con la validación diferida, la fila lleva `🧪 validación diferida a <disparador>` en su lugar — pasa a `✅` cuando el dev-lead dice qué probó ([estados del roadmap](../sdd-start-task/references/control-profiles.md)). Si la task salda una fila de «Deuda técnica» o de «Backlog», ciérrala con el formato de cierre de `roadmap-template.md` (`sdd-templates`): prefijo al principio y texto original intacto. Deuda descubierta → fila en la tabla de deuda técnica.
