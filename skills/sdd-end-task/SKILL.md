...
8. **`roadmap.md`** — marcar el módulo/tarea. `✅` exige validación documentada en el walkthrough; con la validación diferida, la fila lleva `🧪 validación diferida a <disparador>` en su lugar — pasa a `✅` cuando el dev-lead dice qué probó ([estados del roadmap](../sdd-start-task/references/control-profiles.md)). Deuda descubierta → fila en la tabla de deuda técnica.
