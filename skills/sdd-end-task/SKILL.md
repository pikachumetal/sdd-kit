...
1. **`walkthrough.md`** — calcando `walkthrough-template.md` del skill `sdd-templates`. En la verificación, distingue siempre **verificado por ti** (con la evidencia) de **reportado por el usuario**: si la validación se difirió, la línea es `Validación diferida: <fecha> · «<frase literal>» · disparador: <…>`, nunca un «Validado» inventado. En «Decisiones tomadas sin el dev-lead» vuelca los rulings de la ejecución (los «Rulings I made» del informe final de `subagent-driven-development`). Sin verificación documentada — validada o diferida — no hay cierre.
