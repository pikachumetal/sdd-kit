...
10. **Rama** — *(si existe `.docs/sdd/environments.md`)* `env:clean` **ANTES** de invocar `finishing-a-development-branch`: borrar un worktree con el entorno vivo deja contenedores huérfanos secuestrando puertos. Luego `superpowers:finishing-a-development-branch`: verificar estado y aplicar la política de merge a `develop` según el perfil vigente y el bloque `merge` de `sdd-kit.json` ([control-profiles.md](../sdd-start-task/references/control-profiles.md)) — con el bloque ausente o incompleto, o en `pair`, se presenta y se espera, como hoy. Nunca fusiona a `main` ni etiqueta: eso lo decide siempre una persona, en el cierre de la release. Push y creación de PR se confirman siempre, en los tres perfiles.
