...
11. **Ticket para el kit** — si en el paso 0 concretaste tú el disparador de la validación diferida, este mensaje final lo dice primero: «disparador: <…>, a cargo de <…>; lo elegí yo porque tu frase no lo nombraba, corrígelo si no es el tuyo». Luego ofrece generar el ticket de mejora del kit con `sdd-feedback` en esta misma sesión: al limpiar el contexto se pierde lo aprendido sobre el kit. No es un gate — sin respuesta, el cierre termina y no queda nada pendiente ni anotado. Si esta sesión ya generó su ticket, no se ofrece otra vez.
