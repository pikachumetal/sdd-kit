...
3. **Plantilla** — calcar `kit-feedback-template.md` del skill `sdd-templates`.
4. **Guardar** — en `.docs/sdd/kit-feedback/<yyyyMMdd-HHmmss>-(feature|patch)-<id>-<slug>.md`, timestamp UTC, id según el modo del proyecto, igual que las carpetas de spec. Si esta sesión ya tiene ticket, se amplía; no nace un segundo. Si la carpeta no existe, se crea y se avisa **una vez** de que puede ignorarse en git; **no** se edita `.gitignore`.
