...
5. **Handoff cuando se vuelve trabajo** — si la conversación pide hacerlo de verdad, **anúncialo y transiciona al carril**: feature/cambio no trivial → `sdd-start-task`; bug determinista → `sdd-start-patch`; planificar, meter algo en el roadmap o preparar una release → `sdd-roadmap`; investigar un fallo → `superpowers:systematic-debugging`. El carril destino **gatea y asigna el id**. La consulta **no** ejecuta el trabajo por su cuenta.
