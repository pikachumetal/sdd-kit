...
1. **Primar contexto proporcional a la pregunta** — leer los documentos de anclaje relevantes (`mission`, `constitution`, `tech-stack`, `roadmap`, `architecture`, `capabilities/<capacidad>`, actas de `releases/`) antes de responder, y el código si la pregunta lo pide. Proporcionalidad: una duda de estructura lee `architecture` + el código; un "¿qué hacemos ahora?" lee `roadmap` + el acta. **Distingue siempre lo que dice el doc de lo que infieres tú.**
2. **Elige el modo:**
