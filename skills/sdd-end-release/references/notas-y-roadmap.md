...
## Paso 5 — Release notes de cliente
...
5. **Release notes de cliente** *(solo con `release.hasRecipient: true`)* — `.docs/sdd/releases/vX.Y.Z/release-notes.md`,
   ...
   **outcome para el usuario**, no entregable de ingeniería. **Prohibido**: IDs de task, scopes de commit,
...
6. **Colapsar el roadmap** — ANTES de sustituir nada, enumera los pendientes vivos de la sección de la
   ...
   changelog siempre, release notes solo con destinatario, acta solo si existe. Añade también la línea de
   ...
   **hallazgo** = defecto del comportamiento entregado que detecta ese smoke, se corrija o no dentro de la
   release — lo que ya salió en el walkthrough de una task no cuenta. La línea es `smoke: <fecha> · <N>
