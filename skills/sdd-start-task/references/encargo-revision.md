...
La frase corta es la que funcionó en un proyecto real del equipo; la versión larga («sin investigar fuera del repo…») no bastó. Los tests los escribe el hilo principal desde los THEN de la spec antes de despachar, sin commitearlos (paso 6 de `sdd-start-task`); el implementador los hace pasar, no los redacta.
