...
- **La rama lleva el id reservado.** Si al reservar ya estás en una rama sin id (`feature/fix-sala`, la que creó el worktree) y sin commits propios frente a la rama de integración, renómbrala antes del primer commit con `git branch -m feature/<id>-<slug>` y dilo en tu mensaje. Con commits propios no la renombres: avisa y sigue. Sin esta regla, 2 de 2 sujetos del RED reservaron el `0014` y commitearon el fix en `feature/fix-sala` (`tests/sdd-plan-red.md`, p8b).
