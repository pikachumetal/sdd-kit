...
5. **Roadmap como única fuente** *(sin gestor: con gestor, la fuente del scope es él, no el roadmap)* —
   sección "Release N" con filas trazables al acta/origen, en la tabla de `roadmap-template.md`. La celda
   «Ficheros que toca» nombra los ficheros o módulos que la task prevé tocar, leídos del código; si aún no
   existen, la carpeta o el módulo donde irán, nunca «por definir»: el freno de alcance la usa para ver solapes. **No crear
