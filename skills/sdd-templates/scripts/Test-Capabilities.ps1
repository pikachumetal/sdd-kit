...
.DESCRIPTION
  ...
  «# Capacidad — <nombre del fichero>», que solo haya las secciones «Requisitos» y «Reglas de la capacidad», que cada
...
function Test-CapabilitySections([string[]]$Lines) {
  ...
  foreach ($section in $sections | Where-Object { $script:AllowedSections -notcontains $_ }) {
    ...
    "sección «$section» no admitida: solo «Requisitos» y «Reglas de la capacidad»"
  }
}
