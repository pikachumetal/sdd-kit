...
| [scripts/Test-Capabilities.ps1](scripts/Test-Capabilities.ps1) | Valida `capabilities/`: el título, que solo haya «Propósito», «Requisitos» y «Reglas de la capacidad», que el propósito sea la primera sección y no pase de 300 caracteres, que cada requisito tenga `- GIVEN`, `- WHEN` y `- THEN`, que no queden marcas de delta y que las reglas tengan sus cinco entradas. Con `-Artifact`, después de fusionar, comprueba que el bloque «Capacidades» de la spec o del `patch.md` nombra las mismas capacidades que su delta. Lo ejecutan `sdd-end-task` y `sdd-end-patch` tras fusionar: `pwsh -NoProfile -File "<Base directory de sdd-templates>/scripts/Test-Capabilities.ps1" -Path "<raíz del proyecto>/.docs/sdd" [-Artifact "<spec.md o patch.md>"]`. No se copia al proyecto. |
| [scripts/Get-CapabilityIndex.ps1](scripts/Get-CapabilityIndex.ps1) | El índice de `capabilities/`, generado al vuelo y nunca guardado: una línea `` - `<nombre>` — <propósito> `` por capacidad, en orden de nombre, con `(sin propósito)` si le falta. Lo ejecutan `sdd-start-task`, `sdd-roadmap` y `sdd-consult` antes de decidir qué capacidades leer o tocar: `pwsh -NoProfile -File "<Base directory de sdd-templates>/scripts/Get-CapabilityIndex.ps1" -Path "<raíz del proyecto>/.docs/sdd"`. Sale siempre con 0: listar no valida. No se copia al proyecto. |
